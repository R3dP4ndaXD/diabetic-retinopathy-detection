...
JOB_SCRIPT="${JOB_SCRIPT:-${PROJECT_DIR}/scripts/slurm_test_ensemble_apptainer.sh}"
SLURM_OUT_DIR="${SLURM_OUT_DIR:-${PROJECT_DIR}/slurm}"

JOB_NAME="${JOB_NAME:-dr-test-ensemble}"
...
echo "Submitting Slurm ENSEMBLE TEST job with image ${APPTAINER_IMAGE}"
